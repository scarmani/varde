...
**Stagnation.** A move is *quiet* when it is a pass, or a placement that captures nothing and changes no column's control — that is, a placement onto a column the mover already controls. Twelve consecutive quiet moves end the game at once, and this ending is final: resumption may not be demanded. (Placing on an empty point, covering an enemy column, and any capture all reset the count.) This is an operational closure rule, not a proof that strategically meaningful progress was impossible. Evaluation records how often it is needed; frequent stagnation endings are a design smell.
